...
@section('pageTitle', trans( 'Vehicles.page_title_update' ))
    ...
    <div class="col-xs-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title"><i class="icon-head"></i>{{__('vehicles.update_vehicle')}}</h4>
            ...
            <div class="card-body collapse in">
                <div class="card-block">
                    ...
                    @endif
                    <form class="form" id="add-vehicle" action="{{ route('driver_company.vehicle.edit', $vehicle->id) }}"
                        ...
                        <div class="form-body">
                                        ...
                                        <select class="form-control" id="brand" name="brand">
                                            @foreach($brands as $brand)
                                                ...
                                                <option value="{{ $brand->id }}"
                                                        @if($brand->id==old('brand',$vehicle->vehiclemodel->vehiclebrand->id)) selected @endif>
                                                    {{ $brand->name }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="form-actions">
                                <button class="btn btn-danger pull-right" type="submit">
                                    <i class="icon-check"></i> {{__('vehicles.update_vehicle')}}
                                </button>
                            </div>
                                @endif
                        </div>
...
@section('scriptParts')
    <script type="text/javascript">
        ...
        var modelid = "{{$vehicle->vehiclemodel->id}}";
        $(document).ready(function () {
            ...
            $.get('../getmodels/' + $('#brand').val(), function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (parseInt(modelid) === parseInt(data[i].id))
