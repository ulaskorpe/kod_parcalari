    ...
    <div class="modal-body">
        <div class="row">
            <div class="col-md-6">
                ...
                if(!empty($model->locations->first())){
                    $startaddress = json_decode($model->locations->first()->startlocation) ;
                    $startaddress = $startaddress->address;
                    $endaddress=json_decode($model->locations->first()->endlocation);
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="text" id="startadress" name="startadress" class="form-control"
                               value="{{old("startadress",$startaddress)}}" readonly>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="text" id="endadress" name="endadress" class="form-control" value="{{old("endadress",$endaddress)}}" readonly>
                    </div>
                </div>
...
<div class="modal-footer">
    @if(0)
    <a href="javascript:removeJob();" class="btn btn-danger">{{__('orders.remove')}}</a>
    @endif
...
<script type="text/javascript">
    ...
    function getDrivers(){
        ...
        $.get('{{asset('/common/order/find_drivers')}}'+'/'+package+'/'+duration+'/'+$('#orderdate').val()+'/'+$('#driver_id').val(), function (data) {
            $('#driver').html(data);
         //   console.log(data);
        });
    }
    ...
    function updateJob() {
        swal({
            ...
            function (isConfirm) {
                if (isConfirm) {
                    $.post('/common/order/updateform/', $('#formJobUpdate').serialize(), function (mdata, status, xhr) {
                        if (mdata == 'ok') {
                        ...
                        @endrole
                            ...
                            swal("Error!", "It is too late for update job."+mdata, "error");
                        }
                        else {
                        ...
                        toastr["error"](status + " An Error Occurred !"+mdata);
                    });
    ...
    var _date = new Date();
    requestSent = true;
    function loadTimeTable(datetime) {
        _date = datetime;
        $('#loadingTimeTable').show();
        if (!requestSent) {
            requestSent = true;
            $.get('/manager/order/gettimetable/'.concat(formatDate2(datetime)).concat('/').concat($("#is_alldrivers").val().concat('/').concat($("#vehicle_class").val()).concat('/').concat($("#job_vehicle_class").val())), function (mdata) {
            }).done(function (mdata) {
                $('#loadingTimeTable').hide();
                scheduler.clearAll();
                $('#timetablecontent').html(mdata);
                requestSent = false;
            });
        }
    ...
    function getPriceSummary() {
        $.ajax({
            ...
            url: '/common/order/getpricesummary',
